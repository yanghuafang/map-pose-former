...
if (( deploy )); then
  # M7 needs the ONNX path. onnxruntime is what checks an exported graph
  # still computes the same answer as eager PyTorch.
  python -m pip install onnx onnxscript onnxruntime
  if (( cuda )); then
    # M7. Both are Linux/CUDA wheels and have no macOS build, which is

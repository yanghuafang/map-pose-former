...
# what it needs: `tools/cost.py` puts a 4-layer point-token arm at 5006 MiB
# peak at batch 64, and `scripts/run_arms.sh` asks for 8000 MiB before
# admitting one. The difference is the CUDA context and the margin an arm
# claims before it settles.
need="${1:-4500}"

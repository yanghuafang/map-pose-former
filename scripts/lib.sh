...
#
# Sourced by download_nuscenes.sh, and nothing else.
...
#: Where every dataset lands. Deliberately outside the checkout:
#: scripts/remote-ubuntu.sh --sync runs rsync --delete over the checkout and
#: would take a dataset with it. Override MPF_DATA_ROOT to put them on a
#: larger disk; remote-ubuntu.sh exports it for a remote command.
data_root() {
  echo "${MPF_DATA_ROOT:-${HOME}/map-pose-former-data}"
}

#: Pieces fetched at once, per file. Sixteen is measured, not chosen: one
#: connection to S3 sustains 17 kB/s and sixteen sustain 408 kB/s --
...
#
# Bounded by the first non-comment line rather than by a line number: a
# hardcoded `sed -n 'A,Bp'` range is a second copy of where the header ends,
# and the two copies disagree the moment a line is added.
usage() {
...
download_set() {
  ...
  for u in "${urls[@]}"; do
    ...
    # only asks "how big" sums a directory of 404s and hands curl the surprise.

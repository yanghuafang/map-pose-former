...
def _inlier_frac(
    ...
    This is how a matcher decides which detections have a counterpart at
    all, so it is worth asserting on the labels before any model exists: if
    the true correction does not land detections on the map, nothing trained
    against those labels can either.
    """

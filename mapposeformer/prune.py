...
def is_ffn(name: str, mod: nn.Module) -> bool:
    """@brief Is this the feed-forward of a matcher layer?

    A matcher layer's feed-forward is a bare ``nn.Sequential`` rather than a
    class of its own, so ``isinstance`` cannot pick it out -- and *four* other
    Sequentials in the model are not feed-forwards at all:

        map_encoder.fuse      Linear, LayerNorm, GELU, Linear
        det_encoder.point     Linear, LayerNorm, GELU
        det_encoder.subgraph  Linear, LayerNorm, GELU
        matcher...bias.mlp    Linear, GELU, Linear

    The first has a GELU where this code expects its second matrix, so a
    shape-only test fails loudly instead of pruning it. The last is
    structurally identical to a feed-forward and still must not be pruned: it
    is `RelativeBias`, whose output width is the head count, so narrowing its
    hidden layer shrinks a 32-unit table for nothing while coupling the plan
    to the head count.

    So the name decides -- ``.ffn`` is the handle the module tree offers --
    and the shape is asserted, rather than the reverse.
    """
    if not name.endswith(".ffn") or not isinstance(mod, nn.Sequential):
        return False
    ok = (
        len(mod) == 3
        and isinstance(mod[0], nn.Linear)
        and isinstance(mod[2], nn.Linear)
        and mod[0].out_features == mod[2].in_features
    )
    if not ok:
        raise ValueError(
            f"{name} is named like a feed-forward and is not shaped like one: "
            f"{[type(x).__name__ for x in mod]}"
        )
    return True


def ffn_importance(ffn: nn.Sequential) -> Tensor:
...
def prune_ffn(ffn: nn.Sequential, keep: int) -> Tensor:
...
def head_importance(attn: GeometricAttention) -> Tensor:
    ...
    v = attn.v.weight.view(h, d, -1).flatten(1).norm(dim=1)
    out = attn.out.weight.view(-1, h, d).transpose(0, 1).flatten(1)
    return v * out.norm(dim=1)


def prune_heads(attn: GeometricAttention, keep: int) -> Tensor:
    ...
    dim, width = attn.q.in_features, keep * attn.head_dim
    with torch.no_grad():
        for name in ("q", "k", "v"):
        ...
        attn.out = new
    # `relative` mode keeps a learned bias per head; leaving it at the old
    # width would index scores for heads that no longer exist. `rope` and
    # `absolute` have no such table and skip this.
    if getattr(attn, "bias", None) is not None:
        with torch.no_grad():
            for buf in list(attn.bias.parameters(recurse=True)):
                if buf.shape and buf.shape[0] == len(idx) + (attn.heads - keep):
                    buf.data = buf.data[idx].clone()
    attn.heads = keep
    return idx
    ...
    for name, mod in model.named_modules():
        if is_ffn(name, mod):
            ...
            if keep < width:
                prune_ffn(mod, keep)
                plan[name] = keep
        elif isinstance(mod, GeometricAttention):
...
def split_plan(
    ...
    named = dict(model.named_modules())
    ffn = {
        n: w for n, w in plan.items() if isinstance(named.get(n), nn.Sequential)
    }
...
def parameter_count(model: nn.Module) -> tuple[int, int]:
    ...
    prunable = sum(
        ...
        if isinstance(m, (nn.Sequential, GeometricAttention))

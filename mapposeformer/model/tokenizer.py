...
class FourierFeatures(nn.Module):
    ...
    Args:
        num_bands: Wavelengths per axis.
        min_wavelength_m: Finest resolvable detail. Below the detection noise
            floor is wasted capacity and invites aliasing.
        max_wavelength_m: Coarsest band; should exceed the map crop diameter so
            the longest band is monotone across the whole input.
    """
    ...
    def __init__(
        ...
        super().__init__()
        w = torch.logspace(
            math.log10(min_wavelength_m), math.log10(max_wavelength_m), num_bands
        )
...
class PointTokenizer(nn.Module):
    ...
    Each token carries four things: where the point is, what class it belongs
    to, which element it belongs to, and where along that element it sits. The
    element embedding is what turns an unordered bag of points back into a set
    of polylines -- without it, attention cannot tell two lane lines from one.
    """
    ...
    def forward(self, pts: Tensor, pmask: Tensor, cls: Tensor) -> tuple[Tensor, Tensor]:
        """Args are ``(B, N, P, 2)``, ``(B, N, P)`` bool, ``(B, N)`` long.

        Returns:
            ``(tokens (B, N*P, D), pad (B, N*P))`` where ``pad`` is True for
            positions attention must ignore.
        """
        b, n, p, _ = pts.shape
        dev = pts.device

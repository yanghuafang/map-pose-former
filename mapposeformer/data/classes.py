...
``ALONG_TRACK_ANCHORS`` below is the set to switch off.
"""
...
"""What the nuScenes map expansion supplies.

Stated here rather than discovered later in a metrics table, because it is what
the synthetic ablation stands in for. No stop line: the annotation is a stop
*zone* with no usable bar direction. Traffic lights instead, which do carry a
pose -- so the along-track evidence on real data is point-like, not a bar.
"""
...
class MarkType(enum.IntEnum):
    ...
    ``docs/DATASET.md`` has the experiment that this makes possible.

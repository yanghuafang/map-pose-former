...
Both sources emit the same fixed-shape tensors -- ``docs/DATASET.md`` has the
table -- so nothing downstream branches on which one it holds. That is the
...
class DataParams:
    ...
    ``data/nuscenes.py``, and ``docs/ROADMAP.md`` under M5."""
    detections_dir: str = ""
    """Where a detector wrote its output, one ``.npz`` per scene. Empty means
    detections are cut from the map instead -- the synthetic path. See
    ``data/detections.py`` for the format and why the detector cannot run in
    this environment."""
    nuscenes_cache: str = ""
    ...
    cache_dir: str = ""
    """Directory of pre-generated splits; empty means generate on the fly.

    Set it and training reads ``<cache_dir>/<split>.pt`` instead of building
    every sample forty times over. Exact rather than approximate, because
    ``set_epoch`` is never called and all epochs already see identical data --
    wiring ``set_epoch`` up would make the cache wrong. Sequence runs ignore
    it: the closed loop supplies its own prior per frame, which by definition
    cannot be precomputed."""


def build_dataset(
    params: DataParams, split: str, sequential: bool = False
) -> Dataset:
    ...
    """
    # A cache is only valid for callers that want the split as it is. The
    # closed loop asks for a frame under a prior it chose, so `run_sequences`
    # goes to the generator via `sequential=True`.
    if params.cache_dir and not sequential:
        from pathlib import Path

        from mapposeformer.data.cached import CachedDataset

        return CachedDataset(Path(params.cache_dir) / f"{split}.pt")

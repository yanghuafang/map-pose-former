...
convention drops in without a translation layer when M5 reaches a real
detector.
"""
...
#:
#: No poles: nuScenes has none. Measured on a different network, removing
#: them cost 2.3x the longitudinal error -- a figure that sizes the problem
#: rather than predicting this one. Traffic lights are the only point landmark
...
class NuScenesDataset(Dataset):
    ...
    **Detections are still synthetic.** They are cut from this map and corrupted
    ...
    perception. Swapping in a pretrained mapper's output is M5's second half,
    and until that lands the sim-to-real gap is untouched.
    """
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor]:
        ...
        # polylines with noise on top. Replacing them with a detector's own
        # geometry is the second half of M5.
        return build_sample(

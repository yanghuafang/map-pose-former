...
class SyntheticDataset(Dataset):
    ...
    def _world(self, scene: int) -> tuple[World, World]:
        ...
        Training shuffles, so a sampler asks for a different scene at almost
        every index and a one-scene cache misses almost every time -- it would
        only pay under the sequential order that nothing here uses.
    ...
    # access, and they are all a sequence run asks of a source.

...
"""Train a MapPoseFormer.
    ...
    tools/train.py --config configs/synth_smoke.yaml
    tools/train.py --config configs/synth_base.yaml train.lr=1e-3 model.dim=192

Overrides are ``section.field=value`` and are checked against the dataclasses
in ``mapposeformer/config.py``, so a misspelled field fails immediately rather
than training a run that ignored it.
"""

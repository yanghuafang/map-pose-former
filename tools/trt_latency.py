...
def main() -> int:
    ...
    # Positional, not by name. The ONNX exporter names every input `args_N`,
    # so looking that name up in the sample misses every key, and a lookup that
    # falls back to zeros **times every engine on an empty frame**. The export
    # feeds its inputs in the order of the sample keys matching INPUTS, so that
    # order is what reproduces it.
    from tools.export import INPUTS

    ordered = [k for k in sample if k.startswith(INPUTS)]
    names_in = [
    ...
    if len(names_in) != len(ordered):
        raise SystemExit(
            f"engine takes {len(names_in)} inputs, the sample offers "
            f"{len(ordered)}; refusing to time it on partial data"
        )
    ...
    for i in range(engine.num_io_tensors):
        ...
        if engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT:
            key = ordered[names_in.index(name)]
            source = sample[key].unsqueeze(0).numpy().astype(dtype)

...
The distance between red and green is the problem. Nothing in a scalar metric
shows a ridge along the road; this does, and looking at it before trusting a
number is a habit worth forming.
...
def _history(
    ...
    Exactly the composition ``MapPoseFormer.detections`` performs, done here so
    the picture cannot drift from the tensor: if this warp were wrong the orange
...
def render(sample: dict[str, torch.Tensor], span: float) -> str:
    body = [
    ...
    moved = G.transform_points(sample["delta"], all_pts.flatten(0, 1)).view_as(
        all_pts
    )
    body += _draw(moved, all_pmask, all_cls, span, "#188038", 2, 4)
...
def main() -> int:
    ...
    Path(args.out).write_text(render(sample, span))

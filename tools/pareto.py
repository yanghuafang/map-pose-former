...
from mapposeformer.prune import parameter_count
...
def main() -> int:
    ...
    ap.add_argument(
        "--json",
        help="also write the rows here, for whatever plots or publishes them. "
        "A reader that re-parsed the table below would break the moment it "
        "was reformatted.",
    )
    args = ap.parse_args()
    ...
    for spec in args.rows:
        ...
        # `mapposeformer.engine` exports the TRAINER's evaluate, not the
        # evaluator's, and the two return different shapes: the evaluator
        # returns the summary objects, the trainer flattens them to
        # `all/...` and `calib/...` for the metrics log. The flat one is what
        # this needs, and it is what the import already gives.
        #
        # The third column is NEES, not another accuracy variant. A compression
        # frontier ranked on accuracy alone cannot see the failure this project
        # keeps finding, where the pose survives and the covariance does not:
        # pruning to keep 0.50 costs 1.9 pp of recall and a third of the
        # calibration, and only one of those shows up in an accuracy table.
        m = evaluate(model, loader, args.device)
        nees = m.get("calib/anees_median", float("nan"))
        # `trainer.evaluate` leaves the model in train mode on purpose -- it is
        # called mid-training. The latency measured next must not inherit that.
        model.eval()
    ...
    print("| | params | trans | NEES | recall @25cm | p50 | p99 |")
    print("|---|---|---|---|---|---|---|")
